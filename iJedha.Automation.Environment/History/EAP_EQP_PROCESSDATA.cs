...
namespace iJedha.Automation.EAP.Environment
{
    public partial class History
    {
        ...
        public static void EAP_EQP_PROCESSDATA(EquipmentModel em, Socket_DynamicLibraryBase dl)
        {
            try
            {
                if (EAPEnvironment.commonLibrary.MQConnectedStatus == true)
                {
                    return;
                }
                /*
                ...
                */
                ...
                foreach (var item in dl.Dic_ProcessDataModel.Values)
                {
                    #region Oracle
                    _sql = "insert into EAP_EQP_PROCESS(MAIN_EQP_ID,MAIN_EQP_NAME,SUB_EQP_ID,SUB_EQP_NAME,DATE_TIME,ITEM_ID,ITEM_NAME,ITEM_VALUE)" +
                                "values(" +
