...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void EAP_EqpHoldLot(EqpHoldLot _data)
        {
            ...
            try
            {
                ...
                using (ChannelFactory<iJedha.MES.WcfServiceInterface.IWcfServiceContract> channel = new ChannelFactory<iJedha.MES.WcfServiceInterface.IWcfServiceContract>(new NetTcpBinding(SecurityMode.None), new EndpointAddress(EAPEnvironment.commonLibrary.baseLib.wcfParaLibrary.RemoteUrlString)))
                {
                    ...
                    iJedha.MES.WcfServiceInterface.ReturnInformation returnInfo = client.EAP_EquipmentHoldLot(_data.LotID, _data.HoldReason);
                    ...
                    {
                        string errMsg = string.Format("MES回复, 接口名称<{0}>, 错误代码<{1}>, 错误描述<{2}>", System.Reflection.MethodBase.GetCurrentMethod().Name, returnInfo.CheckSuccessCode, returnInfo.CheckErrorMsg);
