...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void EAP_GetEqpUID(EqpUIDRequest _data)
        {
            ...
            try
            {
                ...
                using (ChannelFactory<iJedha.MES.WcfServiceInterface.IWcfServiceContract> channel = new ChannelFactory<iJedha.MES.WcfServiceInterface.IWcfServiceContract>(new NetTcpBinding(SecurityMode.None), new EndpointAddress(EAPEnvironment.commonLibrary.baseLib.wcfParaLibrary.RemoteUrlString)))
                {
                    ...
                    iJedha.MES.WcfServiceInterface.ReturnUIDInformation returnInfo = client.EAP_GetEqpUID(_data.MainEqpID);
                    ...
                    {
                        Environment.EAPEnvironment.commonLibrary.UID = returnInfo.UID;
                        foreach (var v in returnInfo.SubEqpUIDDataList)
                        {
                            EquipmentModel em = EAPEnvironment.commonLibrary.equipmentLibary.GetEquipmentModelByName(v.SubEqpID);
                            if (em != null) em.UID = v.UID;  
                        } 
                    }
                }
            }
