...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void MQ_ProcessError(string alarmcode, string alarmdetail)
        {
            try
            {
                ...
                if (errm == null) return;
                iJedha.Customized.MessageStructure.mProcessError _data = new Customized.MessageStructure.mProcessError();
                ...
                if (Environment.EAPEnvironment.MQPublisherAp.SendMessage_topic(_data, string.Format("{0}.EAP.MQ.ProcessError", Environment.EAPEnvironment.commonLibrary.LineName), out _xmldata))
                {
