...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void MQ_TraceDataCheck(string eqName)
        {
            try
            {
                if (!Environment.EAPEnvironment.commonLibrary.baseLib.rbmqParaLibrary.Enable) return;
                iJedha.Customized.MessageStructure.mTraceDataCheck _data = new Customized.MessageStructure.mTraceDataCheck();
                _data.messagename = "TraceDataCheck";
                ...
                if (Environment.EAPEnvironment.MQPublisherAp.SendMessage_topic(_data, string.Format("{0}.EAP.MQ.TraceDataCheck", Environment.EAPEnvironment.commonLibrary.LineName), out _xmldata))
                {
