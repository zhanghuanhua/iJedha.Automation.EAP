...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void MQ_EquipmentAlarm( string eqname, string alarmtype, string alarmcode, string alarmmessage)
        {
            try
            {
                if (!Environment.EAPEnvironment.commonLibrary.baseLib.rbmqParaLibrary.Enable) return;
                iJedha.Customized.MessageStructure.mEquipmentAlarm _data = new Customized.MessageStructure.mEquipmentAlarm();
                ...
                if (Environment.EAPEnvironment.MQPublisherAp.SendMessage_topic(_data, string.Format("{0}.EAP.MQ.EquipmentAlarm", Environment.EAPEnvironment.commonLibrary.LineName), out _xmldata))
                {
