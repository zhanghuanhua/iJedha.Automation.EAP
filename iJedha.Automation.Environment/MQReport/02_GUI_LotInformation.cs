...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void GUI_LotInformation(string lotid, string pn, string panelcount, string message)
        {
            try
            {
                if (!Environment.EAPEnvironment.commonLibrary.baseLib.rbmqParaLibrary.Enable) return;
                iJedha.Customized.MessageStructure.gLot _data = new Customized.MessageStructure.gLot();
                ...
                if (Environment.EAPEnvironment.GUIMQPublisherAp.SendMessage_topic(_data, string.Format("LotInformation.{0}", Environment.EAPEnvironment.commonLibrary.LineName), out _xmldata))
                {
