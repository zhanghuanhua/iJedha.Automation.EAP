...
namespace iJedha.Automation.EAP.Environment
{
    ...
    {
        public void MQ_LineStatus(string linestatus)
        {
            try
            {
                if (!Environment.EAPEnvironment.commonLibrary.baseLib.rbmqParaLibrary.Enable) return;
                iJedha.Customized.MessageStructure.mLineStatus _data = new Customized.MessageStructure.mLineStatus();
                ...
                if (Environment.EAPEnvironment.MQPublisherAp.SendMessage_topic(_data, string.Format("{0}.EAP.MQ.LineStatus",  Environment.EAPEnvironment.commonLibrary.LineName), out _xmldata))
                {
