...
namespace iJedha.Automation.EAP.WebAPI
{
    public partial class WebAPIReport : BaseComm
    {
        ...
        public void EAP_AlarmReport(MessageModel.AlarmReport _data,int Retrytime)
        {
            ...
            try
            {
                ...
                {
                    string errMsg = string.Format("MES回复, 接口名称<{0}>, 错误代码<{1}>, 错误描述<{2}><{3}>", 
                        System.Reflection.MethodBase.GetCurrentMethod().Name, returnInfo.strCode, _data.SubEqpID, returnInfo.strMsg);
            ...
            catch (Exception e)
            {
                ...
                if (e.Message.ToString() == ConstLibrary.CONST_WEBAPI_TIMEOUT_KEYWORD)
                {
                    ...
                    {
                        Retrytime++;
                        EAP_AlarmReport(_data, Retrytime);
                    }
                    else
                    {
