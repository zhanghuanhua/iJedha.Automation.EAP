...
namespace iJedha.Automation.EAP.WebAPI
{
    public partial class WebAPIReport : BaseComm
    {
        ...
        public void EAP_UnLoadRequest(MessageModel.UnLoadRequest _data,EquipmentModel em, int Retrytime,out string err)
        {
            ...
            try
            {
                ...
                if (ConvertJSON(_data, out _outdata))
                {
                    ...
                    History.EAP_EQP_EVENTHISTORY(em, eEventName.EQP_UNLOADREQUEST, eEventFlow.EAP2MES, $"WebAPI Message <{System.Reflection.MethodBase.GetCurrentMethod().Name}> Send OK", em.CurrentLotID,_data.PortID);
                }
                ...
                if (returnInfo.strCode != "0000")
                {
                    ...
                    err = errMsg;
                    History.EAP_EQP_EVENTHISTORY(em, eEventName.EQP_UNLOADREQUEST, eEventFlow.MES2EAP, errMsg, em.CurrentLotID, _data.PortID);
                    EAP.Environment.BaseComm.ErrorHandleRule("E3005", errMsg, ref errm);
                    return;
                }
                History.EAP_EQP_EVENTHISTORY(em, eEventName.EQP_UNLOADREQUEST, eEventFlow.MES2EAP, "", em.CurrentLotID, _data.PortID);
            }
            catch (Exception e)
            {
                #region [超时及异常处理]
                ...
                if (e.Message.ToString() == ConstLibrary.CONST_WEBAPI_TIMEOUT_KEYWORD)
                {
                    ...
                    {
                        Retrytime++;
                        EAP_UnLoadRequest(_data, em, Retrytime,out errMsg);
                    }
                    else
                    {
